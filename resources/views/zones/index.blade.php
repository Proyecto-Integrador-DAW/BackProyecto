                    ...
                    <table class="min-w-full">
                        <thead>
                            <tr class="divide-x divide-gray-300 dark:divide-gray-700">
                                ...
                                @can('view', \App\Models\Zone::class)
                        ...
                        <tbody class="divide-y divide-gray-300 dark:divide-gray-600">
                            @foreach ($zones as $zone)
                                <tr class="divide-x divide-gray-300 dark:divide-gray-600 dark:bg-[#2c3543]">
                                    ...
                                    {{-- ELIMINADO --}}
                                    @can('view', $zone)
                                        <td class="px-6 py-4 text-md text-gray-800 dark:text-gray-300">{{ $zone->deleted_at }}</td>
                                    @endcan
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex flex-col sm:flex-row gap-2 justify-center items-center w-full sm:w-auto">
                                            ...
                                            @if(!$zone->trashed()) 
                                                ...
                                                @endcan
                                            @else
                                                <form action="{{ route('zones.restore', $zone->id) }}" method="POST">
                                                    @csrf
                                                    @method('PATCH')
                                                ...
                                                <form action="{{ route('zones.forceDelete', $zone->id) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
