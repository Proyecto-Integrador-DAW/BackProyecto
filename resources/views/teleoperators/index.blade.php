    ...
    <div class="py-12 mx-auto sm:px-6 lg:px-8">
        ...
        @if (session('success'))
            <div class="flex items-center p-4 mb-4 text-sm text-green-800 border border-green-300 rounded-lg bg-green-50 dark:bg-gray-800 dark:text-green-400 dark:border-green-800" role="alert">
                ...
                @if (session('id'))
                    <span class="text-lg">&nbsp;Para ver el teleoperador, pulse <a href="{{ route('teleoperators.show', session('id')) }}" class="font-semibold underline hover:no-underline">aquí</a>.</span>
                @endif
            </div>
        ...
        <div class="max-w-9xl">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                @section('create')
                    @can('create', \App\Models\Teleoperator::class)
                        <a href="{{ route('teleoperators.create') }}" class="bg-lime-500 p-2 rounded-[10px]">
                    ...
                    <table class="min-w-full">
                        <thead>
                            <tr class="divide-x divide-gray-300 dark:divide-gray-700">
                                ...
                                <th class="px-6 py-3 text-left font-bold text-gray-800 dark:text-gray-300 bg-gray-200 dark:bg-gray-800 uppercase">Telèfono</th>
                                <th class="px-6 py-3 text-left font-bold text-gray-800 dark:text-gray-300 bg-gray-200 dark:bg-gray-800 uppercase">Zona</th>
                                <th class="px-6 py-3 text-left font-bold text-gray-800 dark:text-gray-300 bg-gray-200 dark:bg-gray-800 uppercase">Código</th>
                        ...
                        <tbody class="divide-y divide-gray-300 dark:divide-gray-600">
                            @foreach ($teleoperators as $teleoperator)
                                <tr class="divide-x divide-gray-300 dark:divide-gray-600 dark:bg-[#2c3543]">
                                    ...
                                    {{-- NOMBRE --}}
                                    <td class="px-6 py-4 text-md text-gray-800 dark:text-gray-300">{{ $teleoperator->name }}</td>

                                    {{-- EMAIL --}}
                                    <td class="px-6 py-4 text-md text-gray-800 dark:text-gray-300">{{ $teleoperator->email }}</td>

                                    {{-- PREFIJO --}}
                                    <td class="px-6 py-4 text-md text-gray-800 dark:text-gray-300">{{ $teleoperator->prefix }} {{ $teleoperator->phone_number }}</td>

                                    {{-- CIUDAD --}}
                                    <td class="px-6 py-4 text-md text-gray-800 dark:text-gray-300">{{ $teleoperator->zone->city ?? 'N/A' }}</td>

                                    {{-- CÓDIGO --}}
                                    <td class="px-6 py-4 text-md text-gray-800 dark:text-gray-300">{{ $teleoperator->code ?? '--' }}</td>

                                    {{-- ELIMINADO --}}
                                    @can('view', $teleoperator)
                                        <td class="px-6 py-4 text-md text-gray-800 dark:text-gray-300">{{ $teleoperator->deleted_at }}</td>
                                    @endcan
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex flex-col sm:flex-row gap-2 justify-center items-center w-full sm:w-auto">
                                            ...
                                            @if(!$teleoperator->trashed()) 
                                                {{-- MOSTRAR --}}
                                                <a href="{{ route('teleoperators.show', $teleoperator->id) }}" class="bg-blue-500 text-white py-2 px-4 rounded-lg shadow hover:bg-blue-600 flex items-center gap-1">
                                                ...
                                                {{-- EDITAR --}}
                                                @can('edit', $teleoperator)
                                                    <a href="{{ route('teleoperators.edit', $teleoperator->id) }}" class="bg-yellow-500 text-white py-2 px-4 rounded-lg shadow hover:bg-yellow-600 flex items-center gap-1">
                                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                                ...
                                                @can('delete', $teleoperator)
                                                    <form action="{{ route('teleoperators.destroy', $teleoperator->id) }}" method="POST" class="inline-block" 
                                                        onsubmit="return confirm('¿Estás seguro de que deseas eliminar este teleoperador? Esta acción no se puede deshacer.');">
                                                        @csrf
                                                        @method('DELETE')
                                                ...
                                                @endcan
                                            @else
                                                <form action="{{ route('teleoperators.restore', $teleoperator->id) }}" method="POST">
                                                    @csrf
                                                    @method('PATCH')
                                                ...
                                                <form action="{{ route('teleoperators.forceDelete', $teleoperator->id) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
                ...
                @else
                    <div class="p-4 text-center text-gray-800 dark:text-gray-300 bg-gray-200 dark:bg-gray-800">
                        <h1 class="text-lg">NO HAY TELEOPERADORES AÚN</h1>
                    </div>
                @endif
            </div>
        </div>
