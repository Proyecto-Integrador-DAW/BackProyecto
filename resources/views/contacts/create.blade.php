...
@section('title', 'Añadir Contacto de Emergencia')

@section('content')
    <h1 class="text-3xl font-bold text-blue-800 mb-6">Añadir Contacto de Emergencia</h1>

    <form action="{{ route('contacts.store') }}" method="POST" class="bg-white p-6 rounded-lg shadow-md max-w-md mx-auto">
        @csrf
        ...
        <div class="mb-4">
            ...
            <input type="text" name="relationship" id="relationship" required
                class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 @error('relationship') border-red-500 @enderror">
            @error('relationship')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
